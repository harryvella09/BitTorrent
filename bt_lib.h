...
#define PRINT(...) if(ok_to_print) printf(__VA_ARGS__)
#define PRINT_LOG(...) if(ok_to_print) logger(0,"tag",__VA_ARGS__)
...
typedef struct bt_msg{
  ...
  union { 
    ...
    unsigned char *data;//pointer to start of payload, just incase
  }payload;

} bt_msg_t;


typedef struct bt_client_args{
	bt_args_t bt_args;
	int clientno;
} pthread_func_args;
...
int send_to_peer(peer_t * peer, bt_msg_t * msg, int sock);

/*read a msg from a peer and store it in msg*/
int read_from_peer(peer_t * peer, bt_msg_t *msg, int sock);
...
/* create a handshake message */
 handshake * create_handshake(unsigned char *, unsigned char *);


int init_handshake(peer_t *,unsigned char *, unsigned char *, int);
int response_handshake(peer_t *,unsigned char *, unsigned char *, int);
...
//static int connections[MAX_CONNECTIONS] = { AVAILABLE, AVAILABLE, AVAILABLE, AVAILABLE, AVAILABLE };
