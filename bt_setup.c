...
void __parse_peer(peer_t * peer, char * peer_st){
  ...
  //find the ip and parse it as a string.
...
//  calc_id(ip,port,id);
  ...
  //Create a thread process to connect to the peer.
...
void parse_args(bt_args_t * bt_args, int argc,  char * argv[]){
  ...
  memset(logfile, 0x00, FILE_NAME_MAX);
  memset(hname, 0x00, INET6_ADDRSTRLEN);
  ...
  while ((ch = getopt(argc, argv, "hp:s:l:vI:b:")) != -1) {
    ...
    case 'v': //verbose
      ...
      break;
	case 'b': //bind
		strncpy(hname, optarg, INET6_ADDRSTRLEN);
		break;
	case 's': //save file
    ...
    case 'I':
      ...
      printf("Peer id will be recalculated based on ipAddress and listening port Number\n");
      break;
    default:
  ...
  strncpy(logfile, bt_args->log_file, FILE_NAME_MAX);
  tfile = fopen(bt_args->torrent_file, "wx");

  
  if (tfile)
  {//exit the program if file doesn't exist!!
	  fclose(tfile);
	  printf("Invalid torrent file : %s\n", bt_args->torrent_file);
	  usage(stderr);
	  exit(1);
  }
